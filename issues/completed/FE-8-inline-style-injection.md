...
## Resolution
Removed the inline `<style>` tag containing `@keyframes spin` from `ProtectedRoute` in `frontend/src/App.jsx`. Added `@keyframes spin` to the global `frontend/src/index.css` file.
