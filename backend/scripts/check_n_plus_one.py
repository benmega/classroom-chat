...
def check_n_plus_one(file_path):
    ...
    failed = False
    if '.query' in content and '.all()' in content and 'joinedload' not in content and 'selectinload' not in content:
        # It's a simplistic check that will just warn for now.
        print(f'[WARN] {file_path}: Found .query.all() without joinedload/selectinload. Verify no N+1 query issues.')
    return failed

if __name__ == '__main__':
    base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'application', 'routes'))
