            ...
            for dec in node.decorator_list:
                if isinstance(dec, ast.Call):
                    if isinstance(dec.func, ast.Attribute) and dec.func.attr == 'route':
                        if isinstance(dec.func.value, ast.Name) and dec.func.value.id == 'admin_bp':
                            is_admin_route = True
                if isinstance(dec, ast.Name):
                    if dec.id in ['admin_only', 'login_required', 'jwt_required']:
                        is_secured = True
            if is_admin_route and not is_secured:
                print(f'[FAIL] {file_path}: Function {node.name} is an admin route but missing security decorator')
                failed = True
    return failed

if __name__ == '__main__':
    base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'application', 'routes', 'admin'))
    has_failures = False
