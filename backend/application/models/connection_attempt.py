...
class ConnectionAttempt(db.Model):
    ...
    parent = db.relationship(
        "User",
        backref=db.backref(
            "connection_attempts", lazy="dynamic", cascade="all, delete-orphan"
        ),
    )
    ...
    def check_rate_limits(parent_id):
        ...
        attempts_15m = (
            ConnectionAttempt.query.filter_by(parent_id=parent_id)
            .filter(ConnectionAttempt.attempted_at >= fifteen_min_ago)
            .count()
        )
        ...
        attempts_today = (
            ConnectionAttempt.query.filter_by(parent_id=parent_id)
            .filter(ConnectionAttempt.attempted_at >= today)
            .count()
        )
    ...
    def log_attempt(parent_id, code, success=False):
        ...
        attempt = ConnectionAttempt(
            parent_id=parent_id, code_attempted=code, success=success
        )
