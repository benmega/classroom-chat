...
def test_check_achievement_types_with_stats(init_db, sample_user):
    ...
    ach_progress = Achievement(name="PR", slug="pr", type="progress", requirement_value="80", source="python")
    ach_chat = Achievement(name="C", slug="c", type="chat", requirement_value="5")
    ach_consistency = Achievement(name="CO", slug="co", type="consistency", requirement_value="3")
    ach_community = Achievement(name="COM", slug="com", type="community", requirement_value="4")
    ach_session = Achievement(name="S", slug="s", type="session", requirement_value="30")
    ach_trade = Achievement(name="T", slug="t", type="trade", requirement_value="6")
    ach_cert = Achievement(name="CR", slug="cr", type="certificate", requirement_value="1")
    ach_default = Achievement(name="DF", slug="df", type="unknown_type", requirement_value="1")

    db.session.add_all([ach_ducks, ach_project, ach_progress, ach_chat, ach_consistency, ach_community, ach_session, ach_trade, ach_cert, ach_default])
    db.session.commit()
...
def test_check_achievement_types_no_stats(init_db, sample_user):
    ...
    ach_community = Achievement(name="COM", slug="com", type="community", requirement_value="2")
    ach_session = Achievement(name="S", slug="s", type="session", requirement_value="15")
...
def test_calculate_consistency(init_db, sample_user):
    ...
    cl1 = ChallengeLog(user_id=sample_user.id, domain="x", challenge_slug="a", timestamp=dt_w1)
    cl2 = ChallengeLog(user_id=sample_user.id, domain="x", challenge_slug="b", timestamp=dt_w2)
    cl3 = ChallengeLog(user_id=sample_user.id, domain="x", challenge_slug="c", timestamp=dt_w4)
    ...
    cl4 = ChallengeLog(user_id=sample_user.id, domain="x", challenge_slug="d", timestamp=dt_w52)
    cl5 = ChallengeLog(user_id=sample_user.id, domain="x", challenge_slug="e", timestamp=dt_w1_2026)
...
def test_evaluate_user(init_db, sample_user):
    ...
    # Setup achievements
    ach1 = Achievement(name="D1", slug="d1", type="ducks", requirement_value="10", reward=5)
    ach2 = Achievement(name="D2", slug="d2", type="ducks", requirement_value="20", reward=10)
...
def test_longest_session_minutes(init_db, sample_user):
    ...
    # Logs
    s1 = SessionLog(user_id=sample_user.id, start_time=datetime.utcnow() - timedelta(minutes=10), end_time=datetime.utcnow())
    s2 = SessionLog(user_id=sample_user.id, start_time=datetime.utcnow() - timedelta(minutes=30), end_time=None) # end_time=None uses utcnow
