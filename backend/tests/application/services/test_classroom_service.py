...
def test_get_enrolled_classroom_ids_with_data(app):
    with app.app_context():
        db.session.execute(
            user_classrooms.insert().values(user_id=1, classroom_id="class_a")
        )
        db.session.execute(
            user_classrooms.insert().values(user_id=1, classroom_id="class_b")
        )
        db.session.execute(
            user_classrooms.insert().values(user_id=2, classroom_id="class_c")
        )
        db.session.commit()
...
def test_user_enrolled_in(app):
    with app.app_context():
        db.session.execute(
            user_classrooms.insert().values(user_id=3, classroom_id="class_d")
        )
        db.session.commit()
