...
def test_submit_trade_not_logged_in(client):
    response = client.post("/duck_trade/submit_trade", data={"digital_ducks": 1}, follow_redirects=True)
    assert response.status_code == 200
    assert b"You must be logged in" in response.data

    response = client.post("/duck_trade/submit_trade", json={"digital_ducks": 1}, headers={"X-Requested-With": "XMLHttpRequest"})
...
def test_submit_trade_invalid_form(client, sample_user_with_ducks, test_app):
    ...
    response = client.post("/duck_trade/submit_trade", data={}, headers={"X-Requested-With": "XMLHttpRequest"})
    assert response.status_code == 400

def test_submit_trade_exception_handling(client, sample_user_with_ducks, test_app, monkeypatch):
    with test_app.app_context():
        DuckTradeLog.query.filter_by(user_id=sample_user_with_ducks.id).delete()
        db.session.commit()
        ...
        response = client.post("/duck_trade/submit_trade", data=form_data, follow_redirects=True)
        assert response.status_code == 200
...
def test_submit_trade_existing_trade_ajax(client, sample_user_with_ducks, test_app):
    with test_app.app_context():
        ...
        assert response.status_code == 400
        assert response.get_json()["message"] == "You already have a pending trade. Please wait for it to be processed."
