...
def test_dev_login_disabled_in_production(client, make_agent_users):
    with patch("application.routes.dev_login_routes._is_dev_environment", return_value=False):
...
def test_dev_login_non_local_blocked(client, make_agent_users):
    with patch("application.routes.dev_login_routes._is_dev_environment", return_value=True):
        resp = client.get("/dev-login?role=admin", environ_overrides={"REMOTE_ADDR": "192.168.1.10"})
        assert resp.status_code == 403
        assert "only accessible from localhost" in resp.json["error"]

        resp = client.get("/api/dev-login?role=admin", environ_overrides={"REMOTE_ADDR": "192.168.1.10"})
        assert resp.status_code == 403
        assert "only accessible from localhost" in resp.json["error"]
