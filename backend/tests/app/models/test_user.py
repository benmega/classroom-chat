...
def test_user_to_dict_auth(add_sample_user, init_db):
    ...
    from application import db
    req = TrackChangeRequest(student_id=user.id, requested_track="test_track", status="pending", requester_type="student")
    db.session.add(req)
    db.session.commit()
...
def test_user_get_contribution_data_with_logs(add_sample_user, init_db):
    ...
    # level 1 (1 log)
    logs.append(ChallengeLog(user_id=user.id, domain="codecombat.com", challenge_slug="a", timestamp=today))
    # level 2 (2 logs)
    logs.extend([ChallengeLog(user_id=user.id, domain="codecombat.com", challenge_slug="b", timestamp=today - timedelta(days=1)) for _ in range(2)])
    # level 3 (5 logs)
    logs.extend([ChallengeLog(user_id=user.id, domain="codecombat.com", challenge_slug="c", timestamp=today - timedelta(days=2)) for _ in range(5)])
    # level 4 (7 logs)
    logs.extend([ChallengeLog(user_id=user.id, domain="codecombat.com", challenge_slug="d", timestamp=today - timedelta(days=3)) for _ in range(7)])
...
def test_user_get_course_progress_data(add_sample_user, init_db):
    ...
    ch = Challenge(slug="test-chal", name="Test Chal", domain="codecombat.com", course_id=course.id)
    db.session.add(ch)
    db.session.commit()
    
    cl = ChallengeLog(user_id=user.id, challenge_slug="test-chal", domain="codecombat.com", course_id=course.id)
    db.session.add(cl)
    db.session.commit()
    
    cl_missing = ChallengeLog(user_id=user.id, challenge_slug="missing-chal", domain="codecombat.com", course_id=course.id)
