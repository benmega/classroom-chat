...
def upgrade():
    ...
    with op.batch_alter_table("users", schema=None) as batch_op:
        ...
        if "cognito_sub" not in existing:
            batch_op.add_column(
                sa.Column("cognito_sub", sa.String(length=50), nullable=True)
            )
        ...
        if "connection_code" not in existing:
            batch_op.add_column(
                sa.Column("connection_code", sa.String(length=10), nullable=True)
            )
        ...
        if "chat_font_color" not in existing:
            batch_op.add_column(
                sa.Column("chat_font_color", sa.String(length=7), nullable=True)
            )
        if "has_animated_border" not in existing:
            batch_op.add_column(
                sa.Column("has_animated_border", sa.Boolean(), nullable=True)
            )
        if "has_auto_bitshift" not in existing:
            batch_op.add_column(
                sa.Column("has_auto_bitshift", sa.Boolean(), nullable=True)
            )
        if "has_custom_wallpaper" not in existing:
            batch_op.add_column(
                sa.Column("has_custom_wallpaper", sa.Boolean(), nullable=True)
            )
        if "profile_wallpaper" not in existing:
            batch_op.add_column(
                sa.Column("profile_wallpaper", sa.String(length=255), nullable=True)
            )
        if "has_auto_claimer" not in existing:
            batch_op.add_column(
                sa.Column("has_auto_claimer", sa.Boolean(), nullable=True)
            )
    ...
    for col, default in new_cols.items():
        if col not in existing:
            bind.execute(
                sa.text(f"UPDATE users SET {col} = {default} WHERE {col} IS NULL")
            )
