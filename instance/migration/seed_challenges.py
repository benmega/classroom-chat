...
def seed_challenges_data(conn, seed_file_path):
    ...
    if not os.path.exists(seed_file_path):
        print(
            f"Seed Notice: Challenges seed file not found at {seed_file_path}. Skipping."
        )
        return
    ...
    try:
        with open(seed_file_path, mode="r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                slug = (
                    row.get("slug", "").strip() or name
                )  # Mimic SQLAlchemy @listens_for

                # Watch for duplicates: Check if a challenge with this name and domain already exists
                cursor.execute(
                    "SELECT 1 FROM challenges WHERE name = ? AND domain = ?",
                    (name, domain),
                )
                if cursor.fetchone():
                    skipped_count += 1
