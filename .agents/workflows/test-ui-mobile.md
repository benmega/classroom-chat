...
3.  **Authentication**: If testing a protected route or a user flow that requires being logged in, **YOU MUST** follow the `@[/login]` workflow. Summary:
    ...
    - **Parent Account Testing**: When reviewing the mobile view for parents, navigate to the standard `/login` form and use the following credentials: username `test_parent` and password `parent123`.
