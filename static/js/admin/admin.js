...
document
  .getElementById("start-conversation-button")
  ?.addEventListener("click", () => {
    postForm(window.urls.startConversationUrl, {})
      .then((data) =>
        showAlert(`Conversation started: ${data.title}`, "success"),
      )
      .catch(console.error);
  });

document
  .getElementById("toggle-message-sending-button")
  ?.addEventListener("click", () => {
...
document
  .getElementById("add-banned-word-button")
  ?.addEventListener("click", () => {
...
document
  .getElementById("update-ducks-button")
  ?.addEventListener("click", () => {
...
document.getElementById("create-user-button")?.addEventListener("click", () => {
  ...
  if (!validUsername)
    return showAlert(
      "Username must be 3–30 chars, lowercase letters, numbers, or underscores only.",
      "error",
    );
...
document
  .getElementById("update-multiplier-button")
  .addEventListener("click", function () {
    const newMultiplier = parseFloat(
      document.getElementById("duck-multiplier-input").value,
    );
    ...
    fetch(window.urls.updateMultiplierUrl, {
      ...
      .then((data) => {
        if (data.success) {
          Swal.fire(
            "Multiplier Updated",
            `New duck multiplier: ${newMultiplier}`,
            "success",
          );
        } else {
          Swal.fire(
            "Error",
            data.message || "Failed to update multiplier.",
            "error",
          );
        }
      })
