...
function Sync-FromProd {
    ...
    # Pull latest prod database using scp via WSL since the key is in WSL
    $WslLocalProdBackup = (wsl wslpath ($LocalProdBackup -replace '\\', '/')).Trim()
    wsl scp -i "$WslSshKey" "${ProdUser}@${ProdHost}:${ProdDbPath}" "$WslLocalProdBackup"
    if ($LASTEXITCODE -ne 0) {
        Write-Host "❌ Error: scp command failed to download the production database."
        exit 1
    }
...
function Restore-DevFromBackup {
    if (-not (Test-Path $LocalProdBackup)) {
        ...
        Write-Host ""
        Write-Host "Run '.\db_sync.ps1 sync' first to create a backup."
        exit 1
    }
...
function Show-Help {
    ...
    if (-not $ScriptName) { $ScriptName = "db_sync.ps1" }
